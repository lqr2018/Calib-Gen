...
def main(args):
    ...
    for i, line in tqdm(enumerate(data), total=len(data), desc="Processing", file=sys.stdout):
        result = context_aware_optimization(line['prompt']['text'], 'pos', args.strength)
        line['prompt']['text'] = result['full_text']
        line['prompt']['origin_text'] = result['original_text']
...
if __name__ == '__main__':
    ...
    parser.add_argument('--prompts_setting', required=True, type=str, choices=toxicity_prompts)
    parser.add_argument('--output_prompts_setting', required=True, type=str, choices=toxicity_prompts)
